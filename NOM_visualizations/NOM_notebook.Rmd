...
processed_data_qc %>%
  filter(assigned_perc > 0.3) -> quality_samples

all_formulae <- quality_samples$molecular_formulae %>%
  unlist() %>%
  unique()
...
holder <- matrix(, nrow = nrow(quality_samples), ncol = length(all_formulae))

for (i in 1:nrow(quality_samples)) {
  holder[i,] <- ifelse(all_formulae %in% quality_samples$molecular_formulae[[i]], 
                       1, 0)
}

quality_samples$data_object_url %>%
  left_join(select(nom_objects_joined_labeled, dobj.url, bsm.id, bsm.sample_type_manual))
