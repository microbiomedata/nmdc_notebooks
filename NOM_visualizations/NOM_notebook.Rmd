...
nom_objects_joined <- nom_biosample_df %>%
  ...
  full_join(rename_with(nom_raw_df, ~ paste0("raw.", .x)), 
            by = join_by(omprc.has_output == raw.was_generated_by),
            keep = TRUE) %>%
  ...
  full_join(rename_with(nom_dobj_df, ~ paste0("dobj.", .x)), 
            by = join_by(analysis.id == dobj.was_generated_by),
            keep = TRUE) %>%
  
  # clean up: remove duplicate rows from joins
  distinct() %>%
...
nom_objects_joined %>%
  select(ends_with("id"), dobj.url, dobj.md5_checksum) %>%
  filter(!is.na(dobj.url)) %>%
  group_by(dobj.md5_checksum) %>%
  filter(n() > 1) %>%
  ungroup()

# at this point I would filter it down to the "real" biosample, but we don't have that becaues was_generated_by isn't populated

# for now, just remove two of em

nom_objects_joined %>%
  filter(!duplicated(dobj.url)) -> nom_objects_joined
...
processed_data_qc <- processed_data_csvs %>%
  mutate(median_confidence  = vapply(csv_nested, function(x) median(x$Confidence.Score, na.rm = TRUE), 
         ...
         unassigned_count   = vapply(csv_nested, function(x) sum(is.null(x$Molecular.Formula)), 
                                     FUN.VALUE = 1, USE.NAMES = FALSE),
         
         assigned_count     = vapply(csv_nested, function(x) sum(!is.null(x$Molecular.Formula)), 
                                     FUN.VALUE = 1, USE.NAMES = FALSE),
         
         assigned_perc      = assigned_count / (unassigned_count + assigned_count),
         
         molecular_formulas = lapply(csv_nested, function(x) unique(x$Molecular.Formula))) %>%
