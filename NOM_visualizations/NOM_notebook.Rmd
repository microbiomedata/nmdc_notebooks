...
## Example of get collection function 
...
One way to compare samples is to investigate what molecular formulae they have in common. We will visualize this by creating a heatmap with a subset of samples of each type, so that our computers do not melt trying to plot 2500 rows of data.
...
rownames(holder) <- samples_for_heatmap$dobj.id
...
#pheatmap(holder, cluster_cols = FALSE, legend = FALSE,
         #annotation_row = sample_type_color_vec)
...
samples_for_heatmap %>%
  ...
  mutate(HC = lapply(csv_nested, function(x) x$H.C),
         OC = lapply(csv_nested, function(x) x$O.C), 
         formula = lapply(csv_nested, function(x) x$Molecular.Formula)) %>%
  select(-c(csv_nested, data_object_url)) %>%
  unnest() %>% 
  filter(formula %in% filtered_formulae) %>% 
  distinct() %>%
