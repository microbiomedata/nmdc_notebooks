...
iterate_file_extract <- function(input_df, identifier_col, url_col, 
  ...
  for (row in 1:nrow(input_df)) {
    ...
    tryCatch(
      ...
      error = function(e){ 
        print(paste("An error occurred fetching data from", identifier, ":", e))
      },
      warning = function(w){
        print(paste("An warning occurred fetching data from", identifier, ":", w))
      },
    )
    ...
    # Filter if specified
    if (!is.na(filter_col) & !is.na(filter_values)) {
